...
### 💾 データ管理
- **2層のデータ永続化戦略**
  - **IndexedDB（ローカル）**: オフライン対応、高速アクセス
  - **Firebase Firestore（クラウド）**: オンライン同期、マルチデバイス対応
- **自動同期**
  - アプリ起動時に自動同期
  - 履歴モーダル表示前に同期
  - オンライン時のCRUD操作で即座に同期
- **オフライン動作**
  - 読み込み: IndexedDBキャッシュで動作
  - 保存/更新/削除: オンライン時のみ可能（整合性保証）
- **履歴管理**
  - 計算結果の保存（商品名・カテゴリー別）
  - カテゴリー優先の保存フロー
  - カテゴリー別フィルタリング
...
**Phase 9リファクタリング完了**: モノリシックな構造から47モジュールの高度にモジュール化されたアーキテクチャへ
...
- **Firebase Firestore**: クラウドデータベース、リアルタイム同期
- **Firebase Authentication**: ユーザー認証（Google, Email/Password）
...
### アーキテクチャ（Phase 9完了、v4.2改善）
- **高度なモジュール化**: 責務ごとに細分化（47モジュール）
...
- **定数化**: UI要素IDや計算定数を一箇所で管理
  - TIME.DEBOUNCE_DELAY = 300ms などの時間定数
- **純粋関数**: 副作用のない計算関数（calculation.js, yield-stats-calc.js）
- **2層データ永続化**: IndexedDB（ローカル） + Firestore（クラウド）
  - Firestoreを信頼できる唯一のソース（Single Source of Truth）
  - IndexedDBはキャッシュとして機能
  - 自動同期でマルチデバイス対応
- **プリセット機能**: 再利用可能なデータパターン管理

### ユーザーエクスペリエンス（Phase 9: UX改善、v4.2改善）
- **段階的表示**: ステップごとに進行を確認
- **即座のフィードバック**: 入力時にリアルタイム計算
  - **デバウンス処理**: 300ms遅延で不要な再計算を抑制（19箇所）
...
- **📊 アイコン付き表示**: 視覚的な情報伝達（🌟✓⚡⚠️📊📈）
- **トースト通知**: 非ブロッキングな通知システム（v4.2）
  - 4種類（info, success, warning, error）
  - グラデーション背景、自動消去（3秒）
  - alert()によるブロッキングUIを廃止（30箇所）
- **エラーガイド**: ユーザーフレンドリーな日本語エラーメッセージ（v4.2）
  - IndexedDBエラーの具体的な対処法を提示
- **逆算機能**: 目標から必要条件を導出
- **スワイプUI**: 直感的な履歴閲覧
...
- **トースト通知システム実装**
  ...
  - グラデーション背景、自動消去（3秒）、アニメーション効果
- **デバウンス処理実装**
  - 入力イベントハンドラーにデバウンス適用（19箇所）
  - 300ms遅延で不要な再計算を削減、パフォーマンス改善
- **IndexedDBエラーハンドリング改善**
  - ユーザーフレンドリーな日本語エラーメッセージ
  - QuotaExceededError、VersionError等に対応
- **重複コード削減**: 約80行削減（保守性向上）
